...
//	SendPropArray( SendPropInt( SENDINFO_ARRAY(m_iPacketloss), 7, SPROP_UNSIGNED ), m_iPacketloss ),
	...
	SendPropArray3( SENDINFO_ARRAY3(m_iObjectiveScore), SendPropInt( SENDINFO_ARRAY( m_iObjectiveScore ), 12 ) ),
END_SEND_TABLE()
...
void CPlayerResource::UpdatePlayerData( void )
{
	for ( int i = 1; i <= gpGlobals->maxClients; i++ )
	{
		...
		if ( pPlayer && pPlayer->IsConnected() )
		{
			...
			if( pHajPlayer )
			{
				m_iClass.Set( i, pHajPlayer->GetCurrentClass() );
				m_iObjectiveScore.Set( i, pHajPlayer->GetObjectiveScore() );
			}
			else
			{
				m_iClass.Set( i, 0 );
