...
/* HAJ 020 - Jed */
//-----------------------------------------------------------------------------
// Does the actual transmission of the concsussion network message to the client.
// 
// CBaseEntity *pEntity  - client entity to send to
// float localAmplitud   - force of the blast
// ConcussCommand_t eCommand - command (start/stop)
//
inline void TransmitConcussionEvent( CBaseEntity *pEntity, float localAmplitude, ConcussCommand_t eCommand )
{
	// just being tidy - set the aplitude to zero if we're transmitting a stop command
	if ( eCommand == CONCUSS_STOP )
		localAmplitude = 0;

	// set the recipient and make it a reliable message	
	CSingleUserRecipientFilter user( (CBasePlayer *)pEntity );

	if( pEntity->IsPlayer() && eCommand != CONCUSS_STOP ) // add all players spectating this player
	{
		for( int i = 0; i < MAX_PLAYERS; i++ )
		{
			CBasePlayer *pPlayer = UTIL_PlayerByIndex( i );

			if ( !pPlayer || pPlayer == pEntity )
				continue;

			if( pPlayer->IsObserver() && pPlayer->GetObserverTarget() == pEntity )
				user.AddRecipient( pPlayer );
		}
	}
	...
	UserMessageBegin( user, "Concuss" );
		WRITE_BYTE( eCommand );
		WRITE_FLOAT( localAmplitude );
	MessageEnd();
	
	DevMsg( "Transmitted Consuss Message. Amplitude %f\n", localAmplitude );
}

/* HAJ 020 - Jed */
//-----------------------------------------------------------------------------
// Purpose: Send the concuss effect to a specific player
//
// Accessor function used to send the client effect. Allows you to add any logic
// before you transmit the network message.
//
// CBaseEntity *pEntity  - client entity to send to
// float localAmplitud   - force of the blast
// ConcussCommand_t eCommand - command (start/stop)
//-----------------------------------------------------------------------------
const float MAX_CONCUSS_AMPLITUDE = 100.0f;		// set and upper limite of 100 for the amplitude

void UTIL_Concuss( CBaseEntity *pEntity, float amplitude, ConcussCommand_t eCommand )
{
	// don't bother if the amplitude has no value.
	if ( eCommand == CONCUSS_START && amplitude <= 0 )
		return;
	...
	// set the upper limit on the concussison amplitude
	if ( amplitude > MAX_CONCUSS_AMPLITUDE )
		amplitude = MAX_CONCUSS_AMPLITUDE;
	
	// call the transmit function
	TransmitConcussionEvent( pEntity, amplitude, eCommand );
}
