...
void CHL2_Player::HandleSpeedChanges( void )
{
	...
	else if( buttonsChanged & IN_WALK )
	{
		if ( IsSuitEquipped() )
		{
			...
			// *HAJ 020* - Jed
			else if( !IsWalking() && !IsSprinting() && (m_afButtonPressed & IN_WALK) && !(m_nButtons & IN_DUCK) && !(m_nButtons & IN_PRONE))
			{
				StartWalking();
			}
		}
...
{
	// *HAJ 020* - Jed Removing a bunch of stuff from HL2
	return false;
}
...
bool CHL2_Player::CanSprint()
{
	return ( m_bSprintEnabled &&										// Only if sprint is enabled 
			...
			// *HAJ 020* - Jed
			( !m_Local.m_bProned || !m_Local.m_bProning || !m_Local.m_bDoProne ) &&			// Nor if we're proning
...
void CHL2_Player::StartWalking( void )
{
	...
	if( !pWeapon || ( pWeapon && !pWeapon->IsIronsighted() ) )
		SetMaxSpeed( HAJ_WALK_SPEED );
...
void CHL2_Player::SuitPower_Update( void )
{
	...
	{
		float flRate = SUITPOWER_CHARGE_RATE;
		...
		SuitPower_Charge( flRate * gpGlobals->frametime );
	}
...
{
	// *HAJ 020* - Jed. Removed a bunch of Hl2 stuff we dont need
}
...
void CHL2_Player::PickupObject( CBaseEntity *pObject, bool bLimitMassAndSize )
{
	...
	// *HAJ 020* - Jed
	// TODO: not sure why but this is causing linker errors.
