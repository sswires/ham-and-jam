...
//	DEFINE_CUSTOM_FIELD( m_Activity, ActivityDataOps() ),
	...
	DEFINE_FIELD( m_flProneTime, FIELD_TIME ),	// *HAJ 020* - Jed
	...
	DEFINE_FIELD( m_bProneToggled, FIELD_BOOLEAN ),
...
int CBasePlayer::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
	...
	/* HAJ *020* - Jed
	// This applies a impact force for bullets but for some reason when prone gets REALLY big values.
	// Seeing as we don't really want bullets knocking back live players, we'll disable this here
	//
...
void CBasePlayer::SetAnimation( PLAYER_ANIM playerAnim )
{
	...
	if (idealActivity == ACT_RANGE_ATTACK1)
	{
		...
		{
			Q_strncpy( szAnim, "prone_shoot_" ,sizeof(szAnim));
		}
	...
	else if (idealActivity == ACT_WALK)
	{
		if (GetActivity() != ACT_RANGE_ATTACK1 || IsActivityFinished())
		{
			...
			// *HAJ 020* - Jed
			if ( GetFlags() & FL_PRONING )	// crouching
			{
				Q_strncpy( szAnim, "prone_aim_" ,sizeof(szAnim));
			}
...
bool CBasePlayer::StartObserverMode(int mode)
{
	...
	RemoveFlag( FL_PRONING );	// *HAJ 020* - Jed
...
void CBasePlayer::CheckObserverSettings()
{
	...
	if (  m_iObserverMode == OBS_MODE_IN_EYE || m_iObserverMode == OBS_MODE_CHASE )
	{
		...
		{
			// *HAJ 020* - Jed
			int flagMask =	FL_ONGROUND | FL_DUCKING | FL_PRONING;
...
// *HAJ 020* - Jed
void CBasePlayer::Prone( )
{
	if (m_nButtons & IN_PRONE) 
	{
		SetAnimation( PLAYER_WALK );
	}
}
...
{
	// *HAJ 020* - Jed
	// Stop Spectators using the "use" key
	if (GetTeamNumber() == TEAM_SPECTATOR)
		ucmd->buttons &= ~IN_USE;
...
{						
	...
	// *HAJ 020* - Jed
	// If trying to prone, already proned, or in the process of proning
	if ( ( m_nButtons & IN_PRONE ) || ( GetFlags() & FL_PRONING ) || ( m_afPhysicsFlags & PFLAG_PRONING ) )
		Duck();
...
// *HAJ 020* - Jed
// Don't send this message as HaJ doesn't have a geiger hud panel
#ifndef HAJ_DLL
...
int CBasePlayer::Restore( IRestore &restore )
{
	...
	if ( GetFlags() & FL_PRONING ) 
	{
		FixPlayerCrouchStuck( this );	// is this right?
		UTIL_SetSize(this, VEC_PRONE_HULL_MIN, VEC_PRONE_HULL_MAX);
		m_Local.m_bProned = true;
	}
...
void CBasePlayer::ImpulseCommands( )
{
	...
	case	201:// paint decal
		...
		if ( gpGlobals->curtime < m_flNextDecalTime || IsObserver() )
		{
			// too early!
			break;
		}
...
// *HAJ 020* - Jed
// HAJ doesn't use the battery hud element
#ifndef HAJ_DLL	
...
void CBasePlayer::Weapon_Equip( CBaseCombatWeapon *pWeapon )
{
	...
	// *HAJ 020* - Jed. Removed a bunch of stuff from HL2 we dont need
...
// *HAJ 020* - Jed
void CBasePlayer::SetupVPhysicsShadow( CPhysCollide *pStandModel, const char *pStandHullName, CPhysCollide *pCrouchModel, const char *pCrouchHullName, CPhysCollide *pProneModel, const char *pProneHullName  )
{
	solid_t solid;
	...
	// create proning hull
	m_pShadowProne = PhysModelCreateCustom( this, pProneModel, GetLocalOrigin(), GetLocalAngles(), pProneHullName, false, &solid );
	...
	{
		SetVCollisionState( VPHYS_PRONING );
	}
	else
	{
...
void CBasePlayer::InitVCollision( void )
{
	...
	//*HAJ 020* - Jed
	CPhysCollide *pProneModel = PhysCreateBbox( VEC_PRONE_HULL_MIN, VEC_PRONE_HULL_MAX );
	SetupVPhysicsShadow( pModel, "player_stand", pCrouchModel, "player_crouch", pProneModel, "player_prone" );
}
...
void CBasePlayer::VPhysicsDestroyObject()
{
	...
	if ( m_pShadowProne )
	{
		PhysDestroyObject( m_pShadowProne );
		m_pShadowProne = NULL;
	}
...
void CBasePlayer::SetVCollisionState( int collisionState )
{
	...
	// *HAJ 020* - Jed
	case VPHYS_PRONING:
		m_pShadowProne->SetPosition( pos, vec3_angle, true );
		m_pShadowProne->SetVelocity( &vel, NULL );
		m_pShadowStand->EnableCollisions( false );
		m_pShadowProne->EnableCollisions( true );
		m_pShadowCrouch->EnableCollisions( false );
		m_pPhysicsController->SetObject( m_pShadowProne );
		VPhysicsSwapObject( m_pShadowProne );
		break;
	
	case VPHYS_NOCLIP:
		m_pShadowCrouch->EnableCollisions( false );
		m_pShadowStand->EnableCollisions( false );
		m_pShadowProne->EnableCollisions( false ); // *HAJ 020* - Jed
		break;
	}
}
...
//-----------------------------------------------------------------------------
// Purpose: Toggle between the prone being on and off
//-----------------------------------------------------------------------------
void CBasePlayer::ToggleProne( void )
{
	// Toggle the state
	m_bProneToggled = !m_bProneToggled;
}

//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
void CC_ToggleProne( void )
{
	CBasePlayer* pPlayer = UTIL_GetCommandClient();
	if ( pPlayer == NULL )
		return;
...
static ConCommand toggle_duck("toggle_prone", CC_ToggleProne, "Toggles prone" );
