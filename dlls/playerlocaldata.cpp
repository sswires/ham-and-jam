...
BEGIN_SEND_TABLE_NOBASE( CPlayerLocalData, DT_Local )
	...
	SendPropBool	(SENDINFO(m_bDoProne)),
	SendPropBool	(SENDINFO(m_bDoUnProne)),
	SendPropBool	(SENDINFO(m_bDuckProne)),

	SendPropFloat	(SENDINFO(m_flNextStanceChange)),
	SendPropFloat	(SENDINFO(m_flNextJumpTime)),

	// vault
	SendPropBool	(SENDINFO(m_bVaulting)),
	SendPropBool	(SENDINFO(m_bVaultingDisabled)),
	SendPropInt		(SENDINFO(m_iVaultHeight), 16 ),
	SendPropInt		(SENDINFO(m_iVaultStage) ),
	SendPropFloat	(SENDINFO(m_flVaultGroundTime)),

	SendPropEHandle (SENDINFO(m_hVaultHint)),
	...
	// *HAJ 020* - Jed
	SendPropFloat	(SENDINFO(m_flPronetime), 12, SPROP_ROUNDDOWN|SPROP_CHANGES_OFTEN, 0.0f, 2048.0f ),
...
BEGIN_SIMPLE_DATADESC( CPlayerLocalData )
	...
	DEFINE_FIELD( m_bProned, FIELD_BOOLEAN ),	// *HAJ 020* - Jed
	DEFINE_FIELD( m_bDoProne, FIELD_BOOLEAN ),
	DEFINE_FIELD( m_bDoUnProne, FIELD_BOOLEAN ),
	DEFINE_FIELD( m_bDuckProne, FIELD_BOOLEAN ),
	DEFINE_FIELD( m_bVaulting, FIELD_BOOLEAN ),
	DEFINE_FIELD( m_bVaultingDisabled, FIELD_BOOLEAN ),
	DEFINE_FIELD( m_hVaultHint, FIELD_EHANDLE ),
	DEFINE_FIELD( m_iVaultHeight, FIELD_INTEGER ),
	DEFINE_FIELD( m_iVaultStage, FIELD_INTEGER ),
	DEFINE_FIELD( m_bInDuckJump, FIELD_BOOLEAN ),
	DEFINE_FIELD( m_flDucktime, FIELD_TIME ),
	DEFINE_FIELD( m_flNextStanceChange, FIELD_TIME ),
