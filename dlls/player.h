...
enum PlayerPhysFlag_e
{
	...
	// *HAJ 020* - Jed
	PFLAG_PRONING		= ( 1<<2 ),		// In the process of proning, but totally squatted yet
	
	PFLAG_USING			= ( 1<<3 ),		// Using a continuous entity
	PFLAG_OBSERVER		= ( 1<<4 ),		// player is locked in stationary cam mode. Spectators can move, observers can't.
	PFLAG_VPHYSICS_MOTIONCONTROLLER = ( 1<<5 ),	// player is physically attached to a motion controller
...
public:
	...
	virtual void			Prone( void );	// *HAJ 020* - Jed
	...
	CBaseCombatWeapon		*Weapon_GetLast( void ) { return m_hLastWeapon.Get(); } // it's in orange box so get over it
...
public:
	...
	// *HAJ 020* - Jed
	void					SetupVPhysicsShadow( CPhysCollide *pStandModel, const char *pStandHullName, CPhysCollide *pCrouchModel, const char *pCrouchHullName, CPhysCollide *pProneModel, const char *pProneHullName );
...
protected: 
	...
	IPhysicsObject				*m_pShadowProne;	// *HaJ 020* - Jed
	Vector						m_oldOrigin;
...
private:
	...
	// *HAJ 020* - Jed
	float					m_flProneTime;		// how long we've been proning
...
// Not transmitted
	...
	bool					m_bProneToggled;		// If true, the player is crouching via a toggle

public:
	bool					GetToggledDuckState( void ) { return m_bDuckToggled; }
	bool					GetToggledProneState( void ) { return m_bProneToggled; }
