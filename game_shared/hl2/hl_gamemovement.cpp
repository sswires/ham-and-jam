...
void CHL2GameMovement::StartForcedMove( bool mounting, float transit_speed, const Vector& goalpos, CFuncLadder *ladder )
{
	...
	lm->m_hReservedSpot = CReservePlayerSpot::ReserveSpot( 
		...
		// *HAJ 020*  - Jed
		GetPlayerMins( ( player->GetFlags() & FL_DUCKING ) ? true : false, ( player->GetFlags() & FL_PRONING ) ? true : false ), 
		GetPlayerMaxs( ( player->GetFlags() & FL_DUCKING ) ? true : false, ( player->GetFlags() & FL_PRONING ) ? true : false ), 
...
void CHL2GameMovement::SetGroundEntity( CBaseEntity *newGround )
{
	...
	else if ( !oldGround && newGround )
	{
		...
		// HaJ jump delay
		player->m_Local.m_flNextJumpTime = gpGlobals->curtime + ( GetHL2Player()->m_HL2Local.m_flSuitPower < 25.0f ? 0.5f : 0.25f );
	}
	else if ( oldGround && !newGround )
	{
