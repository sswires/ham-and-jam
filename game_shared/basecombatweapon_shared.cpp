...
CBaseCombatWeapon::CBaseCombatWeapon()
{
	...
	m_fGrenadeHoldTimer	= 0.0f; // *HaJ 020- Jed* - zero timer for grenades
	...
	m_iClip1 = -1;
	m_iClip2 = -1;
	m_iClipsLeft = 0;	// *HAJ 020*
	m_iPrimaryAmmoType = -1;
	m_iSecondaryAmmoType = -1;

	m_flPrevPrimaryAttack = 0; // *HAJ 020* - Jed
#endif
...
void CBaseCombatWeapon::Precache( void )
{
	...
	if ( ReadWeaponDataFromFileForSlot( filesystem, GetClassname(), &m_hWeaponFileInfo, GetEncryptionKey() ) )
	{
		...
		m_iWorldModelDeployedIndex	= CBaseEntity::PrecacheModel( GetWorldModelDeployed());
...
// Purpose: 
//-----------------------------------------------------------------------------
const char *CBaseCombatWeapon::GetWorldModelDeployed( void ) const
{
	return GetWpnData().szWorldModelDeployed;
}
...
BEGIN_PREDICTION_DATA( CBaseCombatWeapon )
	...
	DEFINE_PRED_FIELD( m_iWorldModelDeployedIndex, FIELD_INTEGER, FTYPEDESC_INSENDTABLE | FTYPEDESC_MODELINDEX ),
	...
	// *HaJ 202* - Jed
	DEFINE_PRED_FIELD( m_iClipsLeft, FIELD_INTEGER, FTYPEDESC_INSENDTABLE ),	
...
BEGIN_DATADESC( CBaseCombatWeapon )
	...
	// *HaJ 202* - Jed
	DEFINE_FIELD( m_iClipsLeft, FIELD_INTEGER ),
...
BEGIN_NETWORK_TABLE_NOBASE( CBaseCombatWeapon, DT_LocalWeaponData )
	...
	// *HaJ 202* - Jed
	SendPropIntWithMinusOneFlag( SENDINFO(m_iClipsLeft ), 8 ),
	...
	// *HaJ 202* - Jed
	RecvPropIntWithMinusOneFlag( RECVINFO(m_iClipsLeft )),
...
BEGIN_NETWORK_TABLE(CBaseCombatWeapon, DT_BaseCombatWeapon)
	...
	SendPropModelIndex( SENDINFO(m_iWorldModelDeployedIndex) ),
	...
	RecvPropInt( RECVINFO(m_iWorldModelDeployedIndex)),
