...
//#1
//Some defines NOTE: this is crappy hacky stuff, should be const float class members
//They go up with the convars and externs under the #includes

#define MIN_GROUND_VAULT_HEIGHT 32.0f
#define MIN_AIR_VAULT_HEIGHT 18.0f
#define MG_VIEW_F_DISP 2.0f
//#define VAULT_USING_LINES 0
...
// Output : const Vector
//-----------------------------------------------------------------------------
const Vector& CGameMovement::GetPlayerMins( bool ducked, bool proned ) const
{
	// *HAJ 020* - Jed
	if ( proned )
		return VEC_PRONE_HULL_MIN;
	if( player->m_Local.IsVaulting() && player->m_Local.m_hVaultHint && player->m_Local.m_hVaultHint->ShouldForceDuck() )
...
// Output : const Vector
//-----------------------------------------------------------------------------
const Vector& CGameMovement::GetPlayerMaxs( bool ducked, bool proned ) const
{
	// *HAJ 020* - Jed
	if ( proned )
...
const Vector& CGameMovement::GetPlayerMins( void ) const
{
	...
	// *HAJ 020* - Jed
	else if ( player->m_Local.m_bProned )
		return VEC_PRONE_HULL_MIN;
...
// Output : const Vector
//-----------------------------------------------------------------------------
const Vector& CGameMovement::GetPlayerViewOffset( bool ducked, bool proned ) const
{
	// *HAJ 020* - Jed
	if( player->m_Local.IsVaulting() && player->m_Local.m_hVaultHint && player->m_Local.m_hVaultHint->ShouldForceDuck() )
		return VEC_DUCK_MOVE_VIEW;

	if ( proned )
		return VEC_PRONE_VIEW;
	else if( ducked )
	{
		if( player->GetAbsVelocity().Length() > 10 )
			return VEC_DUCK_MOVE_VIEW;	
...
void CGameMovement::CheckParameters( void )
{
	...
	// *HAJ 020* - Jed
	if ( !m_bSpeedCropped && ( mv->m_nButtons & IN_SPEED ) && !( player->m_Local.m_bDucked && !player->m_Local.m_bDucking ) && !( player->m_Local.m_bProned && !player->m_Local.m_bProning ) )
	{
	...
	CHAJWeaponBase *pWeapon = dynamic_cast<CHAJWeaponBase*>(player->GetActiveWeapon());

	if( pWeapon )
	{
		float speedf = pWeapon->GetPlayerSpeedMultiplier();

		mv->m_flForwardMove *= speedf;
		mv->m_flSideMove    *= speedf;
		mv->m_flUpMove      *= speedf;
	}
...
void CGameMovement::ReduceTimers( void )
{
	...
	// *HAJ 020* - Jed
	if ( player->m_Local.m_flPronetime > 0 )
	{
		player->m_Local.m_flPronetime -= frame_msec;
		if ( player->m_Local.m_flPronetime < 0 )
		{
			player->m_Local.m_flPronetime = 0;
		}
	}
...
#define PUNCH_SPRING_CONSTANT	120.0f	// bigger number increases the speed at which the view corrects 65.0f
...
void CGameMovement::WaterMove( void )
{
	...
	// *HAJ 020* - Jed
	// Soldiers are usually very heavily weighed down and as such tend to sink like a 
	// rock in water.
	...
	*/
	wishvel[2] -= 90;		// drift towards bottom
	...
	// *HAJ 020* - Jed
	// Slow us down a bit.
	wishspeed *= 0.1; // was 0.8, slow us down a LOT under water.
...
bool CGameMovement::CanVault()
{
	CHajPlayer *pHajPlayer = ToHajPlayer( player );

	if( pHajPlayer )
		return pHajPlayer->CanVault();
...
// Purpose: 
//-----------------------------------------------------------------------------
extern ConVar haj_player_allow_jump;
...
bool CGameMovement::CheckJumpButton( void )
{
	...
	if( haj_player_allow_vault.GetBool() )
	{
		if (player->m_Local.IsVaulting() == false) //Not already vaulting
		{
			if ( mv->m_nOldButtons & IN_JUMP ) //gotta be a fresh button press :)
			{
				return false;		// don't pogo stick
			}
			if( CanVault( ) )
			{
				player->m_Local.m_hVaultHint->StartVault( ToHajPlayer(player) );
				return false;
			}
		}
		else
		{
			mv->m_nOldButtons |= IN_JUMP;
			return false;		// in vault, so no effect
		}
	}


	// don't go any further if jumping is off
	if( !haj_player_allow_jump.GetBool() || player->m_Local.IsVaulting() || gpGlobals->curtime < player->m_Local.m_flNextJumpTime )
		return false;
	...
	{	
		...
		// HAJ 020 - Jed
		// Stop player jumping in water because it corks them to the surface
		// Taking this out means you can only jump out of water waist high.
		/*
	...
	// *HAJ 020* - Jed
	if ( ( player->m_Local.m_bProning || (  player->GetFlags() & FL_PRONING ) ) || player->m_Local.m_bProned )
	{
		if( !player->m_Local.m_bDoUnProne && CanUnprone() && player->m_Local.m_flPronetime <= 0.0f  )
		{
			player->m_Local.m_flPronetime = GAMEMOVEMENT_PRONE_TIME;
			player->ViewPunch( QAngle( 12, 2, 2 ) );	// make the player glance down briefly
			...
			player->m_Local.m_bDuckProne = false;
			player->m_Local.m_bDoUnProne = true;
			ToHajPlayer(player)->DoAnimationEvent( player->m_Local.m_bDuckProne ? PLAYERANIMEVENT_PRONE_TO_CROUCH : PLAYERANIMEVENT_PRONE_TO_STAND );
		}
	...
	// *HAJ 020* - Jed
	// Stop bunny hopping by draining stamina with every jump and
	// only jumping if you have enough left.

	// pointer to the HL2 player class (see HL2_player_shared.h)
	CHL2_Player *pHL2Player = assert_cast<CHL2_Player*>( player );

	if (!pHL2Player)
		return false;

	pHL2Player->m_HL2Local.m_flSuitPower = max( pHL2Player->m_HL2Local.m_flSuitPower - 50, 0 ); //drain stamina for the jump
	...
	if( pHajPlayer )
		pHajPlayer->DoAnimationEvent( PLAYERANIMEVENT_JUMP );
...
bool CGameMovement::CanUnprone()
{
	...
	else
	{
		...
		Vector viewDelta = ( hullSizeNormal - hullSizeProne );
		viewDelta.Negate();
		VectorAdd( newOrigin, viewDelta, newOrigin );
	}

	bool saveproned = player->m_Local.m_bProned;
	...
	player->m_Local.m_bProned = saveproned;
...
void CGameMovement::FinishUnDuck( void )
{
	...
	player->SetViewOffset( GetPlayerViewOffset( false, false ) );
	player->m_Local.m_flDucktime = 0;
...
// *HAJ 020* - Jed
//-----------------------------------------------------------------------------
// Purpose: Stop proning
//-----------------------------------------------------------------------------
void CGameMovement::FinishUnProne( void )
{
	int i;
	trace_t trace;
	Vector newOrigin;
	...
	// Recategorize position since proning can change origin
	CategorizePosition();
}
...
void CGameMovement::FinishUnDuckJump( trace_t &trace )
{
	...
	Vector vecViewOffset = GetPlayerViewOffset( false, false );
...
void CGameMovement::FinishDuck( void )
{
	...
	player->SetViewOffset( GetPlayerViewOffset( true, false ) );
...
// *HAJ 020* - Jed
//-----------------------------------------------------------------------------
// Purpose: Finish proning
//-----------------------------------------------------------------------------
void CGameMovement::FinishProne( void )
{
	int i;
	...
	player->SetViewOffset( GetPlayerViewOffset( false, true ) );
	...
	if ( player->GetGroundEntity() != NULL )
	{
		for ( i = 0; i < 3; i++ )
		{
			mv->m_vecAbsOrigin[i] -= ( VEC_PRONE_HULL_MIN[i] - VEC_HULL_MIN[i] );
		}
	}
	else
	{
...
{
	Vector vDuckHullMin = GetPlayerMins( true, false );
	Vector vStandHullMin = GetPlayerMins( false, false );

	float fMore = ( vDuckHullMin.z - vStandHullMin.z );

	Vector vecDuckViewOffset = GetPlayerViewOffset( true, false );
	Vector vecStandViewOffset = GetPlayerViewOffset( false, player->m_Local.m_bDuckProne );
	...
	{
		temp.z = Approach( vecDuckViewOffset.z, temp.z, gpGlobals->frametime * 50 );
	}
...
// Input  : proneFraction - 
//-----------------------------------------------------------------------------
void CGameMovement::SetPronedEyeOffset( float proneFraction )
{
	Vector vProneHullMin = GetPlayerMins( false, true );
	Vector vStandHullMin = GetPlayerMins( false, false );

	float fMore = ( vProneHullMin.z - vStandHullMin.z );

	Vector vecProneViewOffset = GetPlayerViewOffset( false, true );
	Vector vecStandViewOffset = GetPlayerViewOffset( false, false );
	Vector temp = player->GetViewOffset();
	
	temp.z = ( ( vecProneViewOffset.z - fMore ) * proneFraction ) +
				( vecStandViewOffset.z * ( 1 - proneFraction ) );
...
// Purpose: 
//-----------------------------------------------------------------------------
void CGameMovement::HandleProningSpeedCrop( void )
{
	if ( !m_bSpeedCropped && ( player->GetFlags() & FL_PRONING ) && ( player->GetGroundEntity() != NULL ) )
	{
		float frac = 0.25f;
		mv->m_flForwardMove	*= frac;
		mv->m_flSideMove	*= frac;
...
void CGameMovement::Duck( void )
{
	...
	// check if prone/using bipod
	bool bInProne = ( player->GetFlags() & FL_PRONING ) ? true : false;
	bool bUsingBipod = ( player->GetFlags() & FL_USINGBIPOD ) ? true : false;

	CHAJWeaponBase *pWeapon = dynamic_cast< CHAJWeaponBase* >( player->GetActiveWeapon() );

	// don't allow if proning, using the bipod, dead or spriting.
	CHLMoveData *pMoveData = ( CHLMoveData* )mv;
	if ( bUsingBipod || IsDead() || pMoveData->m_bIsSprinting || player->m_Local.IsVaulting() )
		return;
	...
	if ( ( mv->m_nButtons & IN_DUCK ) || player->m_Local.m_bDucking || bInDuck || bDuckJump || player->m_Local.m_bDuckProne )
	{
		if( player->m_Local.m_bDucking || player->m_Local.m_bDucked )
			SetDuckedEyeOffset( 1.0f );

		// DUCK
		if ( ( mv->m_nButtons & IN_DUCK ) || bDuckJump || ( player->m_Local.m_bDucking && player->m_Local.m_bDuckProne ) )
		{
// XBOX SERVER ONLY
			...
			if ( ( buttonsPressed & IN_DUCK ) && !bInDuck && !bDuckJump && !bDuckJumpTime && ( !( player->m_Local.m_bProning || player->m_Local.m_flPronetime != 0.0f || ( bInProne && !CanUnprone() ) ) ) )
			{
				player->m_Local.m_flDucktime = GAMEMOVEMENT_DUCK_TIME;
				player->m_Local.m_bDucking = true;
				player->m_Local.m_bDuckProne = (bool)( ( player->GetFlags() & FL_PRONING ) > 0 );

				if( player->m_Local.m_bDuckProne && pWeapon && !pWeapon->m_bInReload )
				{
					pWeapon->Holster( NULL );
				}
			...
			if ( player->m_Local.m_bDucking && !bDuckJump && !bDuckJumpTime )
			{
				...
				{
					if( player->m_Local.m_bDuckProne && pWeapon && pWeapon->IsHolstered() )
						pWeapon->Deploy();
		...
		else
		{
			...
			if ( player->m_Local.m_bAllowAutoMovement || bInAir )
			{
				...
				if ( ( buttonsReleased & IN_DUCK ) && !player->m_Local.m_bDuckProne )
				{
				...
				else
				{
					...
					if ( player->m_Local.m_flDucktime != GAMEMOVEMENT_DUCK_TIME && !player->m_Local.m_bProning )
					{
						SetDuckedEyeOffset(1.0f);
						player->m_Local.m_flDucktime = GAMEMOVEMENT_DUCK_TIME;
					}
	...
	else if ( !IsDead() && !player->IsObserver() && !player->m_Local.m_bProning )	// *HAJ 020* - Jed
	{
		if ( ( player->m_Local.m_flDuckJumpTime == 0.0f && player->m_Local.m_flPronetime == 0.0f ) && ( player->GetViewOffset().z != GetPlayerViewOffset( bInDuck, bInProne ).z ) )
		{
			...
			player->SetViewOffset( GetPlayerViewOffset( bInDuck, bInProne ) );
		}
	}
}
...
// *HAJ 020* - Jed
//-----------------------------------------------------------------------------
// Purpose: See if prone button is pressed and do the appropriate things
//-----------------------------------------------------------------------------
void CGameMovement::CheckProne( void )
{
	int buttonsChanged	= ( mv->m_nOldButtons ^ mv->m_nButtons );	// These buttons have changed this frame
	int buttonsPressed	=  buttonsChanged & mv->m_nButtons;			// The changed ones still down are "pressed"
	int buttonsReleased	=  buttonsChanged & mv->m_nOldButtons;		// The changed ones which were previously down are "released"
	...
	// pressed prone
	if ( ( buttonsPressed & IN_PRONE ) && !bInProne && !player->m_Local.m_flPronetime )
	{
		if ( !player->m_Local.m_bProning && !player->GetActiveWeapon()->m_bInReload )
		{
			m_bDoProne = true;
			m_bDoUnProne = false;
		}
	}
	else if ( ( buttonsReleased & IN_PRONE ) && bInProne && !player->m_Local.m_flPronetime )
	{
		if ( !player->m_Local.m_bProning && !player->GetActiveWeapon()->m_bInReload )
		{
			m_bDoProne = false;
...
// *HAJ 020* - Jed
//-----------------------------------------------------------------------------
// Purpose: Handle proning
//-----------------------------------------------------------------------------
void CGameMovement::DoProne( void )
{
	bool bInAir = ( player->GetGroundEntity() == NULL );
	bool bInProne = ( player->GetFlags() & FL_PRONING ) ? true : false;
	bool bUsingBipod = ( player->GetFlags() & FL_USINGBIPOD ) ? true : false;

	// don't allow proning if your in water, using the bipod or dead.
	if ( CheckWater() || bUsingBipod || IsDead() || bInAir )
	{
		m_bDoProne = false;
		return;
	}

	// Have the prone button pressed, but the player currently isn't in the prone position.
	if ( !player->m_Local.m_bProning && !bInProne)
	{
		player->m_Local.m_flPronetime = GAMEMOVEMENT_PRONE_TIME;
		player->m_Local.m_bProning = true;
		player->ViewPunch( QAngle( 22, 2, 2 ) );	// make the player glance down

		CHAJWeaponBase *pWeapon = dynamic_cast< CHAJWeaponBase* >( player->GetActiveWeapon() );

		if( pWeapon && pWeapon->IsIronsighted() )
		{
			pWeapon->ExitIronsights();
		}

		player->ResetSequence( ACT_HAJ_STANDTOPRONE );
	}

	// The player is in prone transition
	if ( player->m_Local.m_bProning )
	{
		float flProneMilliseconds = max( 0.0f, GAMEMOVEMENT_PRONE_TIME - ( float )player->m_Local.m_flPronetime );
		float flProneSeconds = flProneMilliseconds / GAMEMOVEMENT_PRONE_TIME;
			
		// Finish proning immediately if prone time is over
		if ( flProneSeconds > TIME_TO_UNPRONE )
		{
			FinishProne();
			m_bDoProne = false;
		}
		...
		{
			// Calc parametric time
			float flProneFraction = SimpleSpline( flProneSeconds / TIME_TO_PRONE );
			SetPronedEyeOffset( flProneFraction );
		}
	}
}
...
// *HAJ 020* - Jed
//-----------------------------------------------------------------------------
// Purpose: Handle Unproning
//-----------------------------------------------------------------------------
void CGameMovement::DoUnProne( void )
{
	bool bInAir = ( player->GetGroundEntity() == NULL );
	bool bInProne = ( player->GetFlags() & FL_PRONING ) ? true : false;
	bool bUsingBipod = ( player->GetFlags() & FL_USINGBIPOD ) ? true : false;
	...
	if ( player->m_Local.m_bAllowAutoMovement || bInAir )
	{
		// We released the prone button, we aren't in "prone" and we are not in the air - start unprone transition.
		if ( !player->m_Local.m_bProning && bInProne )
		{
			player->m_Local.m_flPronetime = GAMEMOVEMENT_PRONE_TIME;
			player->m_Local.m_bProning = true;
		...
		// Check to see if we are capable of or unproning
		if ( CanUnprone() )
		{
			if ( player->m_Local.m_bProning )
			{
				...
				{
					// Calc parametric time
					float flProneFraction = SimpleSpline( 1.0f - ( flProneSeconds / TIME_TO_UNPRONE ) );
					SetPronedEyeOffset( flProneFraction );
				}
			}
		}
		...
		{
			// Still under something where we can't unprone, so make sure we reset this timer so
			//  that we'll unprone once we exit the tunnel, etc.
			player->m_Local.m_flPronetime = GAMEMOVEMENT_PRONE_TIME;
			player->m_Local.m_bProning = false;
		}
	}
...
void CGameMovement::Prone( void )
{
	...
	// don't allow proning if your in water, using the bipod or dead.
	if ( ( CheckWater() && !bInProne ) || bUsingBipod || IsDead() || player->GetMoveType() == MOVETYPE_LADDER )
		return;

	CHAJWeaponBase *pWeapon = dynamic_cast< CHAJWeaponBase* >( player->GetActiveWeapon() );
	CHajPlayer *pHajPlayer = ToHajPlayer( player );

	// Slow down proned players.
	HandleProningSpeedCrop();

	// If the player is holding down the prone button, the player is in prone transition or proning
	//if ( ( mv->m_nButtons & IN_PRONE ) || player->m_Local.m_bProning || bInProne )
	if( !player->m_Local.m_bDucking && ( buttonsPressed & IN_PRONE ) && player->m_Local.m_flPronetime <= 0.0f && !player->m_Local.IsVaulting() && player->GetGroundEntity() )
	{
		// Have the prone button pressed, but the player currently isn't in the prone position.
		if ( !bInProne )
		{
			...
			player->m_Local.m_bDuckProne = (bool)( ( player->GetFlags() & FL_DUCKING ) > 0 );
			player->m_Local.m_bDoProne = true;
			
			pHajPlayer->StopSprinting();
			pHajPlayer->DoAnimationEvent( player->m_Local.m_bDuckProne ? PLAYERANIMEVENT_CROUCH_TO_PRONE : PLAYERANIMEVENT_STAND_TO_PRONE );

			player->SetMaxSpeed( HAJ_NORM_SPEED );
		}
		else if ( CanUnprone() && buttonsPressed & IN_PRONE && player->m_Local.m_flPronetime <= 0.0f  )
		{
			player->m_Local.m_flPronetime = GAMEMOVEMENT_PRONE_TIME;
			player->ViewPunch( QAngle( 12, 2, 2 ) );	// make the player glance down briefly
			...
			player->m_Local.m_bDuckProne = (bool)( ( player->GetFlags() & FL_DUCKING ) > 0 );
			player->m_Local.m_bDoUnProne = true;
			pHajPlayer->DoAnimationEvent( player->m_Local.m_bDuckProne ? PLAYERANIMEVENT_PRONE_TO_CROUCH : PLAYERANIMEVENT_PRONE_TO_STAND );
		}
	}
			
	// The player is in prone transition
	if ( player->m_Local.m_bDoProne )
	{
		float flProneMilliseconds = max( 0.0f, GAMEMOVEMENT_PRONE_TIME - ( float )player->m_Local.m_flPronetime );
		float flProneSeconds = flProneMilliseconds / GAMEMOVEMENT_PRONE_TIME;

		// Finish in prone transition when transition time is over, in "prone", in air.
		if ( ( flProneSeconds > TIME_TO_PRONE ) || bInAir )
		{
			if( player->GetFlags() & FL_DUCKING )
			{
				player->m_nButtons &= ~IN_DUCK;
				player->RemoveFlag( FL_DUCKING );
			}
			...
			if( pWeapon && pWeapon->IsHolstered() )
			{
				pWeapon->Deploy();
				pWeapon->Ready();
			}

			player->m_Local.m_bDoProne = false;
			return;
		}
		else
		{
	...
	// UNPRONE (or attempt to...)
	else if ( player->m_Local.m_bDoUnProne )
	{
		// We released the prone button, we aren't in "prone" and we are not in the air - start unprone transition.
		// Check to see if we are capable of unducking.
		if ( CanUnprone() )
		{
			...
			if ( flProneSeconds > TIME_TO_UNPRONE )
			{
				...
				player->m_Local.m_bDoUnProne = false;
				return;
			}
			else
			{
...
// Purpose: 
//-----------------------------------------------------------------------------
extern ConVar haj_realism_sprinting_backpedal;
void CGameMovement::PlayerMove( void )
{
	...
	if( !player->m_Local.m_bProned )
	{
		if( !haj_realism_sprinting_backpedal.GetBool() && player->MaxSpeed() > HAJ_NORM_SPEED && mv->m_flForwardMove < 0 )
		{
			mv->m_flForwardMove *= 0.46;
		}
	...
	CBasePlayer *obsplayer = player;

	if( player->IsObserver() && player->GetObserverTarget() )
		obsplayer = ToBasePlayer( player->GetObserverTarget() );

	// TODO add sprint bobbing
	if( obsplayer && ( obsplayer->MaxSpeed() > HAJ_NORM_SPEED || obsplayer->GetGroundEntity() == NULL || ( obsplayer->GetFlags() & FL_PRONING && !obsplayer->m_Local.m_bDoUnProne ) )
		&& obsplayer->GetMoveType() != MOVETYPE_NOCLIP && obsplayer->GetMoveType() != MOVETYPE_OBSERVER )
	{
		float pvel = obsplayer->GetAbsVelocity().Length();

		if( obsplayer->GetFlags() & FL_PRONING )
			player->ViewPunch( QAngle( 0, sin( gpGlobals->curtime * 4 ) * ( pvel / 250 ), 0 ) );
		else
			player->ViewPunch( QAngle( sin( gpGlobals->curtime * 8.5 ) * ( pvel / 9000 ), sin( gpGlobals->curtime * 4.25 ) * ( pvel / 7500 ), sin( gpGlobals->curtime * 8.5 ) * ( pvel / 10000 ) ) );
	}

	// *HAJ 020* - Jed
	// check for status of the prone key and set flags
	//CheckProne();

	// deal with crouching, only if we're not proning
	/*if ( !player->m_Local.m_bProning || ( !player->m_Local.m_bProned && player->m_Local.m_flPronetime <= 0 ) )
	{
		UpdateDuckJumpEyeOffset();
		Duck();
	}*/
	...
	if(player->m_Local.IsVaulting())
	{
		player->SetViewOffset( GetPlayerViewOffset( false, false ) );

		if( player->m_Local.m_hVaultHint != NULL )
			player->m_Local.m_hVaultHint->ProcessVaultMovement( ToHajPlayer(player), mv );
#ifdef GAME_DLL
		else
			player->m_Local.m_bVaulting = false;
#endif
	}


	// prone/unprone based on flags
	//if ( m_bDoProne ) DoProne();
	//if ( m_bDoUnProne ) DoUnProne();
	
	// do the speed crop
	//if ( player->GetFlags() & FL_PRONING || player->m_Local.m_bProning )
	//	HandleProningSpeedCrop();
