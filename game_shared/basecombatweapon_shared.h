...
public:
	...
	virtual const char		*GetWorldModelDeployed( void ) const;
	...
	// *HaJ 020 - Jed*
	// Accessors for the grenade timer variable
	virtual float			GetGrenadeThrowTimer() { return m_fGrenadeHoldTimer; }
	virtual void			SetGrenadeThrowTimer( float t ) { m_fGrenadeHoldTimer = t; }
	...
	// *HaJ 020* - Jed - Accessor for clips left with ammo in them
	int						MagsLeft( void ) { return m_iClipsLeft; }
#ifdef CLIENT_DLL
	float					m_flPrevPrimaryAttack; // *HAJ 020* - Jed
#endif
...
// Client only methods
	...
	virtual void			EnsureCorrectRenderingModel( void );
	...
	virtual int				GetWorldModelDeployedIndex( void );
...
public:
	...
	CNetworkVar( int, m_iWorldModelDeployedIndex );
	// Sounds
...
private:
	...
	float					m_fGrenadeHoldTimer;  // *HAJ 020 - Jed* - how long fire was held before throwing a grenade
...
public:
	...
	// *HaJ 020*  - Jed
	CNetworkVar( int, m_iClipsLeft );				// number of clips containing ammo (can be as little as one round)
