...
const Vector CBasePlayer::GetPlayerMins( void ) const
{
	...
	{
		if ( GetFlags() & FL_DUCKING || m_Local.m_bVaulting )
		{
			return VEC_DUCK_HULL_MIN;
		}
		// *HAJ 020 - Jed Added for proning 
...
void CBasePlayer::UpdateStepSound( surfacedata_t *psurface, const Vector &vecOrigin, const Vector &vecVelocity )
{
	...
	if ( ( GetFlags() & FL_DUCKING) || fLadder )
	{
		...
		velrun = 80;		
		flduck = 100;
		flprone = 0;		// *HAJ 020 - Jed
	}
	// *HAJ 020 - Jed
	else if ( GetFlags() & FL_PRONING )
	{
		velwalk = 30;		// These constants should be based on cl_movespeedkey * cl_forwardspeed somehow
		velrun = 60;		
		flprone = 100;
		flduck = 0;
	}
