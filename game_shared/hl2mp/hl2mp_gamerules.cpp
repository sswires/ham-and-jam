...
#include "ammodef.h"
	...
	#include "haj_cvars.h"
	#include "haj_capturepointmanager.h"
	#include "haj_team.h"
#ifdef DEBUG	
...
BEGIN_NETWORK_TABLE_NOBASE( CHL2MPRules, DT_HL2MPRules )
		...
		RecvPropInt( RECVINFO( m_nCommonwealth ) ), // Jamjar
		RecvPropInt( RECVINFO( m_nAxis ) ),
		RecvPropInt( RECVINFO( m_nYear ) ),
	#else
		SendPropBool( SENDINFO( m_bTeamPlayEnabled ) ),
		SendPropInt( SENDINFO( m_nCommonwealth ) ), // Jamjar
		SendPropInt( SENDINFO( m_nAxis ) ),
...
CHL2MPRules::CHL2MPRules()
{
	...
	// Jam jar entity value placeholders
	m_nCommonwealth = 1;
	m_nAxis = 1;
	m_nYear = 1940;
...
//	float flTimeLimit = mp_timelimit.GetFloat() * 60;
	...
	if ( flFragLimit )
	{
		if( IsTeamplay() == true )
		{
			...
			if ( pAxis->GetScore() >= flFragLimit || pCWealth->GetScore() >= flFragLimit )
			{
				GoToIntermission();
				return;
			}
	...
	// TEMP: check if control point end condition is met
	int nCapturePoints = _capturePointManager.GetCapturePointCount();
...
CAmmoDef *GetAmmoDef()
{
	...
	if ( !bInitted )
	{
		...
		// *HAJ 020* - Jed
		// HaJ ammo types
		def.AddAmmoType("303",				DMG_BULLET,					TRACER_LINE_AND_WHIZ,	0,			0,			500,		0,							0 );
		def.AddAmmoType("303_tracer",		DMG_BULLET,					TRACER_LINE_AND_WHIZ,	0,			0,			500,		0,							0 );
		def.AddAmmoType("9mm",				DMG_BULLET,					TRACER_LINE_AND_WHIZ,	0,			0,			500,		0,							0 );
		def.AddAmmoType("792mm",			DMG_BULLET,					TRACER_LINE_AND_WHIZ,	0,			0,			500,		0,							0 );
		def.AddAmmoType("792mm_tracer",		DMG_BULLET,					TRACER_LINE_AND_WHIZ,	0,			0,			500,		0,							0 );
...
void CHL2MPRules::RestartGame()
{
	...
	// *HAJ 020* - Jed
	// HaJ pointer names
	CTeam *pCWealth = GetGlobalTeam( TEAM_CWEALTH );
	CTeam *pAxis = GetGlobalTeam( TEAM_AXIS );
