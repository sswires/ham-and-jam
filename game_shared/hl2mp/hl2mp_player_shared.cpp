...
void CHL2MP_Player::PlayStepSound( Vector &vecOrigin, surfacedata_t *psurface, float fvol, bool force )
{
	...
	// scale our sound based on your speed. the faster you go the louder you get.
	float speed = clamp( GetLocalVelocity().Length2D(), -200, 200 );
	if ( speed < 0.0f ) speed *= -1;
	float soundscale = RemapVal( speed, 0, 200, 0.25, 1.0f );

	EmitSound_t ep;
	ep.m_nChannel = CHAN_BODY;
	ep.m_pSoundName = params.soundname;
	ep.m_flVolume = fvol * soundscale;
