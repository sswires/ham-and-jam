...
// Input  : *pSetup - 
//-----------------------------------------------------------------------------
ConVar haj_cl_tpfovoffset( "haj_cl_tpfovpitchoffset", "5", FCVAR_CLIENTDLL );
ConVar haj_cl_tpfovzoffset( "haj_cl_tpfovzoffset", "0.5", FCVAR_CLIENTDLL );
...
void ClientModeShared::OverrideView( CViewSetup *pSetup )
{
	...
	if( ::input->CAM_IsThirdPerson() )
	{
		...
		// haj tp offset
		Vector camOrigin = pSetup->origin;

		int fov = pPlayer->GetFOV();
		float offset = 90 - fov;

		camOrigin.z += ( offset * haj_cl_tpfovzoffset.GetFloat() );
		camAngles[ PITCH ] -= offset / haj_cl_tpfovoffset.GetFloat();

		VectorMA( camOrigin, -cam_ofs[ ROLL ], camForward, pSetup->origin );
...
void ClientModeShared::FireGameEvent( IGameEvent *event )
{
	...
	else if ( Q_strcmp( "player_team", eventname ) == 0 )
	{
		...
		if ( team < 4 )	// *HAJ 020* Dont print team name if selected Random
		{
