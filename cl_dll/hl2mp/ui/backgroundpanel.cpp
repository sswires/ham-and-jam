...
void ApplyBackgroundSchemeSettings( EditablePanel *pWindow, vgui::IScheme *pScheme )
{
	...
	// Logo -----------------------------------------------
	pBitmapPanel = dynamic_cast< CBitmapImagePanel * >(pWindow->FindChildByName( "GameLogo" ));
	if ( pBitmapPanel )
	{
...
static void FixupBackgroundPanels( EditablePanel *pWindow, int offsetX, int offsetY )
{
	...
	// *HAJ 020* - Jed
	// Better values for no-square logo. Assuming HAJ logo width = 4x height;
	int logoHeight = GetAlternateProportionalValueFromNormal( 40 ); // we want 80 px high as the 640x480version
	int logoWidth = logoHeight * 4;
	int logoInsetX = GetAlternateProportionalValueFromNormal( 400 );
	int logoInsetY = GetAlternateProportionalValueFromNormal( 72 );

	// Background panel
	int bgHeight = GetAlternateProportionalValueFromNormal( 360 );
	int bgWidth  = (int)( (double)bgHeight * 1.444f ); // 520/360 = 1.444
	int bgInsetX = GetAlternateProportionalValueFromNormal( 60 );
	int bgInsetY = GetAlternateProportionalValueFromNormal( 60 );
	...
	{
		pPanel->SetZPos( -20 );
		pPanel->SetBounds( 0, 0, screenWide, bgInsetY );
	}
	...
	{
		pPanel->SetZPos( -20 );
		pPanel->SetBounds( 0, screenTall - bgInsetY, screenWide, bgInsetY );
	}
	...
	{
		pPanel->SetZPos( -20 );
		pPanel->SetBounds( 0, bgInsetY, bgInsetX, screenTall - 2*bgInsetY );
	}
	...
	{
		pPanel->SetZPos( -20 );
		pPanel->SetBounds( screenWide - bgInsetX, bgInsetY, bgInsetX, screenTall - 2*bgInsetY );
	}

	// Background -----------------------------------------
	pPanel = pWindow->FindChildByName("HaJBackground");
	if ( pPanel )
	{
		pPanel->SetZPos( -20 );
		pPanel->SetBounds( offsetX + bgInsetX, offsetY + bgInsetY, bgWidth, bgHeight );
	}

	// Logo -----------------------------------------------
	pPanel = pWindow->FindChildByName( "GameLogo" );
	if ( pPanel )
	{
		pPanel->SetZPos( -19 ); // higher than the background
		pPanel->SetBounds( offsetX + logoInsetX, offsetY + logoInsetY, logoWidth, logoHeight );
	}
}
...
void CreateBackground( EditablePanel *pWindow )
{
	...
	// Logo -----------------------------------------------
	new CBitmapImagePanel( pWindow, "HaJBackground", "gfx/vgui/menu_background" );
	new CBitmapImagePanel( pWindow, "GameLogo", "gfx/vgui/haj_logo" );
}
