...
ConVar haj_cl_allow_motd( "haj_cl_allow_motd", "1", FCVAR_CLIENTDLL + FCVAR_ARCHIVE, "Show motds?");
...
void CHL2MPTextWindow::OnKeyCodePressed(KeyCode code)
{
	...
	{
		IViewPortPanel *pCwealthClassMenu = gViewPortInterface->FindPanelByName( PANEL_HAJCLASS_CWEALTH );
		IViewPortPanel *pAxisClassMenu = gViewPortInterface->FindPanelByName( PANEL_HAJCLASS_AXIS );

		if( !( ( pCwealthClassMenu && pCwealthClassMenu->IsVisible() == true ) ||
			( pAxisClassMenu && pAxisClassMenu->IsVisible() == true ) ) )
		{
