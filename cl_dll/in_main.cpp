...
// *HAJ 020* Jed - Global pront toggle bool
...
kbutton_t			in_prone;	// *HAJ 020* - Jed
...
static  kbutton_t   in_showobjloc;
...
void IN_ProneDown(void) { KeyDown(&in_prone); }
void IN_ProneUp(void) { KeyUp(&in_prone); }
...
float g_fLastProne = 0.0f;

void IN_ProneToggle(void)
{
	C_HajPlayer *pLocalPlayer = C_HajPlayer::GetLocalHajPlayer();

	if( g_fLastProne + 1.5 > gpGlobals->curtime || gpGlobals->curtime >= g_fLastProne + 1.5 )
	{
		if ( ::input->KeyState( &in_prone ) )
		{
			KeyUp( &in_prone ); 
		}
		else
		...
		g_fLastProne = gpGlobals->curtime;
	}
}
...
void IN_ScoreDown(void)
{
	...
	{
		IViewPortPanel *pClassMenu = gViewPortInterface->FindPanelByName( PANEL_HAJCLASS );

		if( pClassMenu && pClassMenu->IsVisible() == false )
		{
...
// =====================================
// Hand the show objective locations key
// =====================================

extern ConVar cl_drawobjectives;

void IN_StartObjectiveLocations( void )
{
	KeyDown( &in_showobjloc );
	cl_drawobjectives.SetValue( 1 );
}

void IN_EndObjectiveLocations( void )
{
	KeyUp( &in_showobjloc );
	cl_drawobjectives.SetValue( 0 );
}
...
int CInput::GetButtonBits( int bResetState )
{
	...
	CalcButtonBits( bits, IN_SHOWOBJLOC, s_ClearInputState, &in_showobjloc, bResetState );
...
// *HAJ 020* - Jed
static ConCommand toggleprone_d("+prone", IN_ProneDown);
static ConCommand toggleprone_u("-prone", IN_ProneUp);
...
// HAJ *020* Objective location overlay
static ConCommand startobjectiveshowinfo("+objectiveoverlay", IN_StartObjectiveLocations );
static ConCommand endobjectiveshowinfo("-objectiveoverlay", IN_EndObjectiveLocations );
