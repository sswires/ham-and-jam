...
void CBaseHudChatLine::ApplySchemeSettings(vgui::IScheme *pScheme)
{
	...
	// *HAJ 020* - Jed
	// Use different colour definition for text
	m_clrText = pScheme->GetColor( "Chat.TextColor", GetFgColor() );
...
void CBaseHudChatInputLine::ApplySchemeSettings(vgui::IScheme *pScheme)
{
	...
	m_pPrompt->SetFgColor( pScheme->GetColor( "Chat.PromptColor", GetFgColor() ) );
...
void CBaseHudChatInputLine::PerformLayout()
{
	...
	m_pPrompt->SetBounds( 2, 0, w, tall );

	m_pInput->SetBounds( w + 4, 0, wide - w - 2 , tall );
}
...
void CBaseHudChat::OnTick( void )
{
	...
	if ( line )
	{
		...
		if ( font )
		{
			...
			// Put input area at bottom
			// *HAJ 020* - Jed
			// This is where we offset the input box!
			int w, h;
			GetSize( w, h );
			m_pChatInput->SetBounds( 8, h - m_iFontHeight - 8, w - 8, m_iFontHeight );
		}
	}
	...
	int currentY = h - m_iFontHeight - 8;	// *HAJ 020* - Jed dont forget to set the inset here!
	int startY = currentY;
	...
	for ( i = CHAT_INTERFACE_LINES - 1; i >= 0 ; i-- )
	{
		...
		line->PerformFadeout();
		// *HAJ 020* - Jed
		// This is where we offset the chat lines!
		line->SetSize( w - 8, m_iFontHeight * line->GetNumLines() );
		line->SetPos( 7, ( currentY+m_iFontHeight) - m_iFontHeight * line->GetNumLines() );
