...
//-----------------------------------------------------------------------------
// Purpose: returns true if current weapon has its bipod deployed
//-----------------------------------------------------------------------------
bool CBaseHudWeaponSelection::UsingWeaponBipod( void )
{
	C_BasePlayer *player = C_BasePlayer::GetLocalPlayer();
	if ( !player )
		return false;

	if ( player->GetFlags() & FL_USINGBIPOD )
		return true;
...
void CBaseHudWeaponSelection::SelectSlot( int iSlot )
{
	...
	if ( !BaseClass::ShouldDraw() || UsingWeaponBipod() )
	{
		return;
	}
