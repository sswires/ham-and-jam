...
static ConVar r_screenfademinsize( "r_screenfademinsize", "0", FCVAR_CHEAT );
...
ConVar haj_cl_drawteamsprites( "haj_cl_drawteamsprites", "1", FCVAR_CLIENTDLL + FCVAR_ARCHIVE, "Show overhead sprites?"); 

//-----------------------------------------------------------------------------
// Purpose: Shows team icons above teammates' heads
// Input  : none
//-----------------------------------------------------------------------------
void CViewRender::RenderTeamSprites()
{
	if( !haj_cl_drawteamsprites.GetBool() )
		return;
	...
	{	
		...
		if( pPlayer->IsPlayerDead() || pPlayer->IsObserver() )
			continue;

		C_BasePlayer *pLocalPlayer = C_BasePlayer::GetLocalPlayer();

		// *HaJ* voice: don't show icon for player on opposite team
		if( pLocalPlayer && pLocalPlayer->GetTeamNumber() != pPlayer->GetTeamNumber() && pLocalPlayer->GetTeamNumber() != TEAM_SPECTATOR )
			continue;

		pPlayer->DrawTeamSprite();
	}
}
