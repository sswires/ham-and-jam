...
{
	// HaJ: Shouldn't show scoreboard if class menu is visible.
	IViewPortPanel *pCwealthClassMenu = m_pViewPort->FindPanelByName( PANEL_HAJCLASS_CWEALTH );
	IViewPortPanel *pAxisClassMenu = m_pViewPort->FindPanelByName( PANEL_HAJCLASS_AXIS );

	if( bShow && ( ( pCwealthClassMenu && pCwealthClassMenu->IsVisible( ) == true ) || ( pAxisClassMenu && pAxisClassMenu->IsVisible( ) == true ) ))
	{
