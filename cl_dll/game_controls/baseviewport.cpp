...
// *HAJ 020* - Jed
// Show the team memu
CON_COMMAND( chooseteam, "Show the team menu" )
{
	if ( !gViewPortInterface )
		return;
...
CON_COMMAND( chooseclass, "Show the class menu" )
{
	...
	C_HL2MP_Player *pPlayer = dynamic_cast< C_HL2MP_Player*>( player );

	if( !pPlayer ) return;

	// *HaJ 202* - SteveUK
	// Made it so nothing shows when you're not on a team and try and select a class.
	if ( player->GetTeamNumber() == TEAM_SPECTATOR && !( pPlayer->GetDesiredTeam() == TEAM_AXIS || pPlayer->GetDesiredTeam() == TEAM_CWEALTH ) )
	{
		return;
	}

//	if ( player->GetTeamNumber() == TEAM_BRITISH ) 
//	{
		if ( !gViewPortInterface )
			return;
		gViewPortInterface->ShowPanel( PANEL_HAJCLASS, true );
//	}
//	else if ( player->GetTeamNumber() == TEAM_GERMANS) 
//	{
//		if ( !gViewPortInterface )
//		return;
//		gViewPortInterface->ShowPanel( "axis_class", true );
//	}
}
// END HAJ
*/
...
CON_COMMAND( voicemenu, "Opens a menu for voice coms" )
{
   if ( !gViewPortInterface )
      return;
   gViewPortInterface->ShowPanel( PANEL_VOICE, true );
}
...
void CBaseViewport::CreateDefaultPanels( void )
{
	...
	AddNewPanel( CreatePanelByName( PANEL_HAJCLASS ) );
	AddNewPanel( CreatePanelByName( PANEL_VOICE ) );

	AddNewPanel( CreatePanelByName( PANEL_HAJCLASS_AXIS ) );
	AddNewPanel( CreatePanelByName( PANEL_HAJCLASS_CWEALTH ) );

	// AddNewPanel( CreatePanelByName( PANEL_OVERVIEW ) );
	// AddNewPanel( CreatePanelByName( PANEL_BUY ) );

	//CBaseViewport::CreatePanelByName( PANEL_OVERVIEW );
#endif
}
...
IViewPortPanel* CBaseViewport::CreatePanelByName(const char *szPanelName)
{
	...
	else if ( Q_strcmp(PANEL_HAJCLASS, szPanelName) == 0 )
	{
		newpanel = new CHAJClassSelection( this );
	}
	else if ( Q_strcmp(PANEL_HAJCLASS_AXIS, szPanelName) == 0 )
	{
		newpanel = new CHAJAxisClassMenu( this );
	}
	else if ( Q_strcmp(PANEL_HAJCLASS_CWEALTH, szPanelName) == 0 )
	{
		newpanel = new CHAJCwealthClassMenu( this );
	}
