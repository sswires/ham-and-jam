...
void CSpectatorMenu::ApplySchemeSettings(IScheme *pScheme)
{
	...
	m_pConfigSettings->SetFont( pScheme->GetFont("SpectatorFont", IsProportional() ) );
	m_pViewOptions->SetFont( pScheme->GetFont("SpectatorFont", IsProportional() ) );
	m_pPlayerList->SetFont( pScheme->GetFont("SpectatorFont", IsProportional() ) );
}
...
void CSpectatorMenu::OnKeyCodePressed(KeyCode code)
{
	...
	if( !cl_drawhud || !cl_drawhud->GetBool() )
		return;
...
void CSpectatorGUI::ApplySchemeSettings(IScheme *pScheme)
{
	...
	m_pTopBar->SetBgColor( Color( 0,0,0,0 ) );
	m_pBottomBarBlank->SetBgColor( Color( 0, 0, 0, 0));
	// m_pBottomBar->SetBgColor(Color( 0,0,0,0 ));
	SetPaintBorderEnabled(false);
	SetBorder( NULL );

	// *HAJ 020* - Jed - Get the we want to use
	m_hFont = pScheme->GetFont("SpectatorFont", true );
	//GetLabelByName( "respawntime" )->SetFont( pScheme->GetFont( "CenterPrintText", IsProportional() ) );
	
	// HACK - set font for team labels. (Move to generic "SetFonts" function?)
	GetLabelByName("CTScoreLabel")->SetFont(m_hFont);
	GetLabelByName("TERScoreLabel")->SetFont(m_hFont);

	m_pPlayerLabel->SetBgColor( BLACK_BAR_COLOR );
	m_pPlayerLabel->SetContentAlignment( vgui::Label::a_center );
	m_pPlayerLabel->SetFont( m_hFont );
...
void CSpectatorGUI::SetLabelText(const char *textEntryName, wchar_t *text, bool changefont)
{
	...
	if (entry)
	{
		...
		if( changefont != false )
		{
			entry->SetFont( m_hFont ); // *HAJ 020* - Jed - Set Font
		}
	}
}
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Gets the text of a control by name
//-----------------------------------------------------------------------------
vgui::Label *CSpectatorGUI::GetLabelByName(const char *textEntryName)
{
	Label *entry = dynamic_cast<Label *>(FindChildByName(textEntryName));
	if ( entry )
		return entry;
...
void CSpectatorGUI::ShowPanel(bool bShow)
{
	...
	{
		// *HAJ 020* - Jed
		// Stop scorebored popping up in spectator mode.
		//m_bSpecScoreboard = false;
...
void CSpectatorGUI::Update()
{
	...
	*/
	...
	if ( playernum > 0 && playernum <= gpGlobals->maxClients && gr )
	{
		...
		m_pPlayerLabel->SizeToContents();

		int x, y, w, h;
		m_pPlayerLabel->GetBounds( x, y, w, h );
		m_pPlayerLabel->SetBounds( (ScreenWidth()/2) - (w/2) - 10, y, w + 20, h + 20 );
	}
	else
	{
...
/*
	...
	//
	// Update scores
	int timer = pGameRules->GetRoundTimeLeft();
	_snwprintf ( szText, sizeof( szText ), L"%d:%02d", (timer / 60), (timer % 60) );
	// END HAJ

	C_HL2MP_Player *pLocalPlayer = C_HL2MP_Player::GetLocalHL2MPPlayer();

	if( pLocalPlayer && ( pLocalPlayer->GetTeamNumber() == TEAM_CWEALTH || pLocalPlayer->GetTeamNumber() == TEAM_AXIS ) )
	{
		int remainingtime = (int)( pGameRules->GetSpawnTimeForTeam( pLocalPlayer->GetTeamNumber() ) - gpGlobals->curtime );

		if( pGameRules->IsInOvertime() )
		{
			if( pGameRules->IsSuddenDeath() )
				_snwprintf ( szRespawn, sizeof( szRespawn ), L"No spawning in sudden death", remainingtime );
			else
				_snwprintf ( szRespawn, sizeof( szRespawn ), L"No spawning in overtime", remainingtime );
		}
		else if( pGameRules->IsSuddenDeath() && !pGameRules->IsInOvertime() )
			_snwprintf ( szRespawn, sizeof( szRespawn ), L"" );
		else if( remainingtime >= 0 )
			_snwprintf ( szRespawn, sizeof( szRespawn ), L"Reinforcements in %d", remainingtime );
	}

	szText[63] = 0;

	SetLabelText("timerlabel", szText );
	//SetLabelText("respawntime", szRespawn, false );
*/
}
...
{
    wchar_t cwTeamScore[6], axTeamScore[6];
    C_Team *teamCWealth = GetGlobalTeam( TEAM_CWEALTH );
    C_Team *teamAxis = GetGlobalTeam( TEAM_AXIS );

    if ( teamCWealth )
	{
        swprintf( cwTeamScore, L"%d", teamCWealth->Get_Score() );
        SetLabelText( "CTScoreValue", cwTeamScore );
		SetLabelText( "CTScoreLabel", teamCWealth->Get_Name() );
    }
    if ( teamAxis )
	{
        swprintf( axTeamScore, L"%d", teamAxis->Get_Score() );
        SetLabelText( "TERScoreValue", axTeamScore );
		SetLabelText( "TERScoreLabel", teamAxis->Get_Name() );
    }
}
// END HAJ
