...
protected:
	...
	void SetLabelText(const char *textEntryName, wchar_t *text, bool changefont);
	...
	void SetLogoImage(const char *image);
	 ...
	 // *HAJ 020* - Jed
	void UpdateScores();
	vgui::Label *GetLabelByName(const char *textEntryName); // Gets a pointer to a VGUI label
	vgui::HFont	m_hFont; // font
	// END HAJ
