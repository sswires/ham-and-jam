...
IMPLEMENT_CLIENTCLASS_DT_NOBASE(C_PlayerResource, DT_PlayerResource, CPlayerResource)
	...
	RecvPropArray3( RECVINFO_ARRAY(m_iObjectiveScore), RecvPropInt( RECVINFO( m_iObjectiveScore[0]))),
END_RECV_TABLE()
...
//	memset( m_iPacketloss, 0, sizeof( m_iPacketloss ) );
	...
	memset( m_iObjectiveScore, 0, sizeof( m_iObjectiveScore ) );
	...
	// *HAJ 020* - Jed 
	// HaJ Team colours
	m_Colors[TEAM_AXIS] = Color( 200, 50, 50, 255 ); //COLOR_RED;
	m_Colors[TEAM_CWEALTH] = Color( 140, 155, 85, 255 ); //COLOR_GREEN;
#endif
...
int C_PlayerResource::GetPlayerClass( int iIndex )
{
	if( !IsConnected( iIndex ) )
		return 0;
...
int C_PlayerResource::GetObjectiveScore( int iIndex )
{
	if( iIndex < 0 || iIndex > MAX_PLAYERS )
		return 0;

	return m_iObjectiveScore[iIndex];
}

float C_PlayerResource::GetCombatEffectiveness( int iIndex )
{
	if( iIndex < 0 || iIndex > MAX_PLAYERS )
		return 0;

	float iScore = (float)GetPlayerScore( iIndex );
	float iDeaths = (float)GetDeaths( iIndex );

	if( iDeaths == 0 )
		iDeaths = 1;

	float iCE = iScore / iDeaths;
