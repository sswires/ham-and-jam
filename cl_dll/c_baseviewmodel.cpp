...
void C_BaseViewModel::FireEvent( const Vector& origin, const QAngle& angles, int event, const char *options )
{
	...
	if ( pWeapon )
	{
		// *HaJ* hack for muzzle flash
		if( pWeapon->m_bInReload && ( event == AE_MUZZLEFLASH || event == CL_EVENT_EJECTBRASS1 ) )
			return;
...
bool C_BaseViewModel::Interpolate( float currentTime )
{
	...
	float dt = elapsed_time * ( GetSequenceCycleRate( pStudioHdr, GetSequence() ) * m_flPlaybackRate );
	if ( dt >= 1.0f )
	{
