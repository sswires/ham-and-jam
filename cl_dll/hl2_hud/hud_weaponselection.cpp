			...
			for ( int i = 0; i < MAX_WEAPON_SLOTS; i++ )
			{
				...
				else
				{
					...
					if ( GetFirstPos( i ) )
					{
						...
						if( pWeapon && pWeapon->GetSpriteInactive() )
						{
							...
							float iconWide = ( m_flSmallBoxSize - 10 );

							if( (iconWide/aspect) > m_flSmallBoxSize - 5 )
							{
								iconWide = ( m_flSmallBoxSize - 10 ) * aspect; 
							}

							Color col = Color( 255, 255, 255, m_flAlphaOverride );
							if (!pWeapon->CanBeSelected() || !pWeapon->HasAnyAmmo() )
							{
								// unselectable weapon, display as such
								col = Color(255, 0, 0, m_flAlphaOverride );
							}

							// draw the inactive version
							pWeapon->GetSpriteInactive()->DrawSelf( xpos + 5, ypos + 25, iconWide, iconWide/aspect, col );
						}
					}
					else
					{
...
void CHudWeaponSelection::DrawLargeWeaponBox( C_BaseCombatWeapon *pWeapon, bool bSelected, int xpos, int ypos, int boxWide, int boxTall, Color selectedColor, float alpha, int number )
{
	...
	case HUDTYPE_BUCKETS:
		{
			...
			if ( pWeapon->GetSpriteActive() )
			{
				...
				float flAspect = iconWidth / iconHeight;

				if( iconWidth + 10 > boxWide )
				{
					iconWidth = boxWide - 20;
					iconHeight = iconWidth / flAspect;
				}

				if( iconHeight + 10 > boxTall )
				{
					iconHeight = boxTall - 20;
					iconWidth = iconHeight * flAspect;
				}
				...
				else if (bSelected)
				{
					...
					pWeapon->GetSpriteActive()->DrawSelf( xpos + x_offs, ypos + y_offs, iconWidth, iconHeight, col );
				}
	...
	if ( bSelected )
	{
		...
		int tx = xpos + boxWide - m_flSelectionNumberXPos - firstslen;
		...
		for (wchar_t *pch = text; charCount > 0; pch++)
		{
			if (*pch == '\n')
			{
				...
				surface()->DrawSetTextPos( xpos + boxWide - m_flSelectionNumberXPos - slen, ty + (surface()->GetFontTall(m_hTextFont) * 1.1f));
			}
			else if (*pch == '\r')
			{
				// do nothing
...
void CHudWeaponSelection::DrawBox(int x, int y, int wide, int tall, Color color, float normalizedAlpha, int number)
{
	...
	if (number >= 0)
	{
		...
		surface()->DrawSetTextPos(x + m_flSelectionNumberXPos, y + tall - m_flSelectionNumberYPos);
