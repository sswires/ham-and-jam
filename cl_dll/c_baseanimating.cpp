...
void C_BaseAnimating::MaintainSequenceTransitions( CStudioHdr *hdr, float flCycle, float flPoseParameter[], Vector pos[], Quaternion q[], int boneMask )
{
	...
	// *HAJ 020* - Jed
	// Found on HLCoders. Meant to help with animations snapping back
...
void C_BaseAnimating::ProcessMuzzleFlashEvent()
{
	...
	if ( muzzleflash_light.GetBool() )
	{
		...
		if ( m_Attachments.Count() > 0 )
		{
			...
			// Make an elight
			// *HaJ 020* - SteveUK - Use dynamic light instead!!!
			// Jed - doubled up so it lights the world and ents.
			...
			dl->radius = random->RandomFloat( 128.0f, 192.0f );
			...
			el->color.g = 192;
			el->color.b = 64;
			el->radius = random->RandomFloat( 128.0f, 192.0f );
...
bool C_BaseAnimating::DispatchMuzzleEffect( const char *options, bool isFirstPerson )
{
	...
	if ( token ) 
	{
		...
		{
			weaponType = MUZZLEFLASH_HAJ_PISTOL;
		}
		else if ( Q_stricmp( token, "RIFLE" ) == 0 )
		{
			weaponType = MUZZLEFLASH_HAJ_RIFLE;
		}
		else if ( Q_stricmp( token, "SMG" ) == 0 )
		{
			weaponType = MUZZLEFLASH_HAJ_SMG;
		}
		else if ( Q_stricmp( token, "LMG" ) == 0 )
		{
			weaponType = MUZZLEFLASH_HAJ_LMG;
		}
		else if ( Q_stricmp( token, "HMG" ) == 0 )
		{
			weaponType = MUZZLEFLASH_HAJ_HMG;
		}
...
void C_BaseAnimating::FireEvent( const Vector& origin, const QAngle& angles, int event, const char *options )
{
	...
	case AE_MUZZLEFLASH:
		{
			// don't do if in thirperson mode
