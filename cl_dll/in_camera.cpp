...
==============================
*/
extern ConVar haj_allowthirdperson;
extern const ConVar *sv_cheats;
void CAM_ToThirdPerson(void)
{
	if( ( sv_cheats && !sv_cheats->GetBool() ) && !haj_allowthirdperson.GetBool() )
		return;

	CHajGameRules *pRules = HajGameRules();

	if( pRules && pRules->IsGameOver() )
		return;
...
void CInput::CAM_Think( void )
{
	...
	if ( !((sv_cheats && sv_cheats->GetBool()) || haj_allowthirdperson.GetBool()) || ( pRules && pRules->IsGameOver() ) || ( pPlayer && ( !pPlayer->IsAlive() || pPlayer->IsObserver() ) ) )
	{
		CAM_ToFirstPerson();
		return;
	}
...
// HAJ 020 - Jed
// Take out this limits on using thirdperson mode. Now its sv_cheats only.
/*
...
{
	if( haj_allowthirdperson.GetInt() == 2 )
	{
		CHajGameRules *pRules = HajGameRules();
		C_HajPlayer *pPlayer = C_HajPlayer::GetLocalHajPlayer();

		if ( ( pRules && pRules->IsGameOver() ) || ( pPlayer && ( !pPlayer->IsAlive() || pPlayer->IsObserver() ) ) )
			return false;
