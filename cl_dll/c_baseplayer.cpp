...
BEGIN_RECV_TABLE_NOBASE( CPlayerLocalData, DT_Local )
	...
	RecvPropBool	(RECVINFO(m_bDoProne)),
	RecvPropBool	(RECVINFO(m_bDoUnProne)),
	RecvPropBool	(RECVINFO(m_bDuckProne)),

	RecvPropFloat	(RECVINFO( m_flNextStanceChange )),
	RecvPropFloat	(RECVINFO( m_flNextJumpTime ) ),

	RecvPropBool	(RECVINFO(m_bVaulting)),
	RecvPropBool	(RECVINFO(m_bVaultingDisabled)),
	RecvPropInt		(RECVINFO(m_iVaultHeight)),
	RecvPropInt		(RECVINFO(m_iVaultStage)),
	RecvPropFloat	(RECVINFO(m_flVaultGroundTime)),

	RecvPropEHandle (RECVINFO(m_hVaultHint)),

	RecvPropInt		(RECVINFO(m_bInDuckJump)),
	RecvPropFloat	(RECVINFO(m_flDucktime)),

	RecvPropFloat	(RECVINFO(m_flPronetime)),	// *HAJ 020* - Jed
...
BEGIN_PREDICTION_DATA_NO_BASE( CPlayerLocalData )
	...
	DEFINE_PRED_FIELD( m_bDoProne, FIELD_BOOLEAN, FTYPEDESC_INSENDTABLE ),
	DEFINE_PRED_FIELD( m_bDoUnProne, FIELD_BOOLEAN, FTYPEDESC_INSENDTABLE ),

	DEFINE_PRED_FIELD( m_bVaulting, FIELD_BOOLEAN, FTYPEDESC_INSENDTABLE ),
	DEFINE_PRED_FIELD( m_iVaultHeight, FIELD_INTEGER, FTYPEDESC_INSENDTABLE ),
	DEFINE_PRED_FIELD( m_iVaultStage, FIELD_INTEGER, FTYPEDESC_INSENDTABLE ),
	DEFINE_PRED_FIELD( m_flVaultGroundTime, FIELD_INTEGER, FTYPEDESC_INSENDTABLE ),
	...
	// *HAJ 020* - Jed
	DEFINE_PRED_FIELD( m_flPronetime, FIELD_FLOAT, FTYPEDESC_INSENDTABLE ),
...
void C_BasePlayer::CalcChaseCamView(Vector& eyeOrigin, QAngle& eyeAngles, float& fov)
{
	...
	if ( player && player->IsAlive() )
	{
		...
		// *HAJ 020* - Jed
		else if( player->GetFlags() & FL_PRONING )
		{
...
void C_BasePlayer::CalcInEyeCamView(Vector& eyeOrigin, QAngle& eyeAngles, float& fov)
{
	...
	if( engine->IsHLTV() )
	{
		...
		// *HAJ 020* - Jed
		else if ( target->GetFlags() & FL_PRONING )
		{
